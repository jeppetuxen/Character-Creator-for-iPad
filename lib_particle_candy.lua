...
----------------------------------------------------------------
local SetEmitterTarget = function (emitterName, FollowTarget, followRotation, followRotationOffset)
...
local Update = function ()
	...
	local emitterX, emitterY, emitterRot, emitterScale, emitterAlpha, EmitterParent
	...
	for i,eName in ipairs(V.EmitterIndex) do
		...
		if Emitter.active then
			...
			if Emitter.FollowObject ~= nil then
				emitterX = Emitter.FollowObject.x
				emitterY = Emitter.FollowObject.y
				Emitter.x = emitterX
				Emitter.y = emitterY
